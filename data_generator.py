...
def rnd_data_point(maxinc, maxexc, fprs=None, job_type="series"):
    ...
    if job_type == "series":
        ...
        bitstrings, duration = generate_data_series(
            n_included, n_excluded, maxinc, maxexc, fprs
        )
        ...
        delta = random.sample(
            valid, random.randint(1, min(maxinc - n_included, len(valid)))
        )
        revoked2 = revoked + delta
        valid2 = [x for x in valid if x not in delta]
        different = cascadeUtils.create_padded_cascade(
            revoked2, valid2, maxinc, maxexc, fprs, multi_process=True
        )
        duration = time.time() - start_time
        return (
            get_cascade_bitstrings(cascade),
...
def standardize_dataset(
    data, max_cascades, max_length, job_type="series", batch_size=1000
):
    """
...
def save_to_csv_with_padding(
    data, filename, job_type="series", padding=False, batch_size=1000
):
    """
    ...
    if padding:
        ...
        standardized_generator = standardize_dataset(
            data, max_cascades, max_length, job_type, batch_size
        )
    else:
        if job_type == "series":
            standardized_generator = (
                data[0][i : i + batch_size] for i in range(0, len(data[0]), batch_size)
            )
        elif job_type == "classification":
            standardized_generator = (
                (data[0][i : i + batch_size], data[1][i : i + batch_size])
    ...
    with open(filename, "w", newline="") as csvfile:
        ...
        if job_type == "series":
            writer.writerow(
                ["concatenated_bitstrings", "num_included", "num_excluded", "duration"]
            )
            X, y = data
            sample_index = 0
                ...
                for sample in batch:
                    ...
                    if padding:
                        ...
                        concatenated_bitstring = ",".join(
                            ...
                            for cascade in X[sample_index]
                        )
                    num_included, num_excluded, duration = y[sample_index]
                    writer.writerow(
                        [concatenated_bitstring, num_included, num_excluded, duration]
                    )
                    sample_index += 1
            ...
            writer.writerow(
                [
                    "concatenated_bitstrings_X1",
                    "concatenated_bitstrings_X2",
                    "label",
                    "duration",
                ]
            )
            ...
            for batch in tqdm(standardized_generator, desc="Processing batches"):
                ...
                for sample1, sample2 in zip(batch1, batch2):
                    ...
                    if padding:
                        ...
                        concatenated_bitstring1 = ",".join(
                            "".join(format(byte, "08b") for byte in cascade)
                            for cascade in X1[sample_index]
                        )
                        concatenated_bitstring2 = ",".join(
                            "".join(format(byte, "08b") for byte in cascade)
                            for cascade in X2[sample_index]
                        )
                    label, duration = y[sample_index]
                    writer.writerow(
                        [
                            concatenated_bitstring1,
                            concatenated_bitstring2,
                            label,
                            duration,
                        ]
                    )
...
def main():
    ...
    if job_type not in ["series", "classification"]:
        raise ValueError(
            "Invalid job_type in config. Must be 'series' or 'classification'."
        )
    ...
    fname = os.path.join(
        CONFIG["output_directory"], f"training-data-{job_type}-{time.time_ns()}.csv"
    )
    print(f"Saving data to {fname}...")

    save_to_csv_with_padding(
        d, fname, job_type=job_type, padding=CONFIG["padding"], batch_size=1000
    )

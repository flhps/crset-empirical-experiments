...
def rnd_data_point(maxinc, maxexc, fprs):
    ...
    bitstrings, duration, tries = generate_data_series(
    ...
    with open(filename, "w", newline="") as csvfile:
        ...
        for data, maxinc, maxexc, margin, fpr in tqdm(all_data, desc="Saving to CSV"):
            ...
            for sample_X, sample_y in zip(X, y):
                total_bitstring_length = sum(
                    len(cascade) * 8 for cascade in sample_X
                )  # Each byte is 8 bits
                writer.writerow(
                    [maxinc, maxexc, margin, fpr] + sample_y + [total_bitstring_length]
                )
...
def main():
    ...
    maxinc_maxexc_pairs = [
        (1000, 1000)
    ]  # [(1000, 100_000), (100_000, 100_000), (100_000, 1000)]
    fprs = np.arange(0.01, 0.99, 0.01).round(
        2
    )  # From 0.1% to 10% with steps of 0.1% (100 steps)
    margins = [1.05]
            ...
            for margin in margins:
                for fpr in fprs:
                    d = generate_benchmark_data(
                        n_samples=samples, maxinc=maxinc, maxexc=maxexc, fprs=[fpr]
                    )
    ...
    fname = os.path.join(
        CONFIG["output_directory"], f"benchmarking-data-{time.time_ns()}.csv"
    )
